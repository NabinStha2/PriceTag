...
public class ApplicationExceptionHandler {
    ...
    public ResponseEntity<CommonResponseDto<Void>> handleApplicationException(final ApplicationException exception,
        ...
        return new ResponseEntity<>(
                buildErrorResponse(exception.getMessage(), exception.getHttpStatus(), exception, request),
    ...
    public ResponseEntity<CommonResponseDto<Void>> handleExpiredJwtException(final ExpiredJwtException exception,
                                                                             final HttpServletRequest request) {
        return new ResponseEntity<>(
                buildErrorResponse("JWT token has expired. Please login again.", HttpStatus.UNAUTHORIZED, exception,
                                   request),
    ...
    public ResponseEntity<CommonResponseDto<Void>> handleUnknownException(final Exception exception,
                                                                          final HttpServletRequest request) {
        return new ResponseEntity<>(buildErrorResponse("An unexpected error occurred. Please contact support.",
                                                       HttpStatus.INTERNAL_SERVER_ERROR, exception, request),
                                    HttpStatus.INTERNAL_SERVER_ERROR);
    }

    private CommonResponseDto<Void> buildErrorResponse(String message, HttpStatus status, Exception exception,
        ...
        Map<String, Object> meta = Map.of("guid", guid, "path", request.getRequestURI(), "method", request.getMethod(),
